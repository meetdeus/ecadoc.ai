...
const axisForDirection = (direction: Direction) => (direction === 'left' || direction === 'right' ? 'X' : 'Y');
...
export const RevealController = () => {
  ...
  useEffect(() => {
    ...
    const elements = Array.from(document.querySelectorAll<HTMLElement>('[data-ns-animate]'));
    if (!elements.length) return;

    elements.forEach(prepareElement);

    const instantElements = elements.filter((el) => el.hasAttribute('data-instant'));
